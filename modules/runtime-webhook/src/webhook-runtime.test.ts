...
import { webhookRuntimeInternals, runtimeWebhookModule } from "./index.ts";
import { capabilityNames, createDomainEvent, type ModuleContext } from "@wato/core";
...
test("webhook runtime delivers and retries events", async () => {
  ...
  const registration = await runtimeWebhookModule.register(context);
