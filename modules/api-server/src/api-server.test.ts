...
import { capabilityNames, type ModuleContext } from "@wato/sdk";
...
test("api handler lists labels", async () => {
  ...
  const handler = createApiFetchHandler({
    context,
    accounts: { list: () => [] } as never,
    storage: { listMessages: () => [], listWorkflows: () => [], listWorkflowExecutions: () => [], listWebhookDeliveries: () => [], getWebhookDelivery: () => undefined, upsertAccounts() {}, saveEvent() {}, getEvent() { return undefined; }, saveInboundMessage() {}, saveOutboundMessage() {}, saveWorkflow() {}, saveWorkflowExecution() {}, saveWebhook() {}, deleteWebhook() {}, listWebhooks: () => [], saveWebhookDelivery() {} },
    system: { getStatus: () => ({ name: "test", status: "ready", uptimeMs: 1, moduleCount: 1, accountCount: 0 }) },
    sender: { async sendText() { return { messageId: "1" }; } },
    gateway,
