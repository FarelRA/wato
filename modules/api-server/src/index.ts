...
import type { MessageSender, StorageEngine, SystemController, WebhookRegistry, WatoModule, WhatsAppGateway } from "@wato/sdk";
import { capabilityNames } from "@wato/sdk";

export const apiServerModule: WatoModule = {
  manifest: {
    name: "api-server",
    version: "0.1.0",
    kind: "integration",
    dependsOn: ["whatsapp-core", "storage-archive", "workflow-core"],
    provides: ["api-router"],
  ...
  register(context) {
    ...
    context.capabilities.register("api-router", {
  ...
  return async (request: Request): Promise<Response> => {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/messages") {
        return Response.json({ messages: storage.listMessages(url.searchParams.get("accountId") ?? undefined, 100) });
      }
      ...
      if (request.method === "GET" && url.pathname === "/workflow-providers") {
        const registry = context.capabilities.resolve<{ listProviderTypes?: () => { triggers: string[]; conditions: string[]; actions: string[] } }>("workflow-registry");
      ...
      if (request.method === "POST" && url.pathname === "/workflows/validate") {
        const registry = context.capabilities.resolve<{ validate?: (workflow: WorkflowDefinition) => { ok: boolean; issues: string[] } }>("workflow-registry");
      ...
      if (request.method === "POST" && url.pathname === "/workflows") {
        const registry = context.capabilities.resolve<{ upsert?: (workflow: WorkflowDefinition) => void }>("workflow-registry");
      ...
      if (request.method === "POST" && url.pathname === "/messages/info") {
        return Response.json(await gateway.getMessageInfo(await parseBody(request)));
      }
